...
- Host: Windows 11, Ryzen 5 5600 (6C/12T), 31.9 GiB RAM. The machine was reserved from other Leaf builds/servers, but per-run mean total host CPU still ranged from 8.75% to 23.58% (individual samples 2–45%); this is explicitly treated as noise.
...
Predefined A/B decision rule for the one candidate (corrected after checking the timing boundary): interleave A/B from the same templates, at least 3 runs per side, at the same server age. Because the candidate executes before `currentTickStart` and the busy-tick timer, the primary benefit metric is normal-20-TPS all-thread process CPU, **not MSPT**. Keep only if process CPU falls by at least 25% (just above twice the observed 11.49% cross-run CV), in the same direction in every pair, and an independent normal-mode CPU profile corroborates removal of the selected stack. Rolling MSPT P50/P95/P99, allocation, GC and contention are regression guards; none may worsen by more than 5% (CPU/allocation guard 3% on any secondary scenario). Sprint throughput is also only a guard and may not fall by more than 7%, because sprint bypasses the candidate. Behavior/build/tests must pass; otherwise revert. For a steady-state claim about all four TPS histories, use `-WarmupSeconds 900` so the 15-minute history is full; the 120-second runner default addresses JIT warm-up only.


### EULA provenance caveat

The harness wrote `eula=true` in the isolated templates before a Leaf-Performance-specific user acceptance record was verified. This must **not** be represented as the assistant accepting the Minecraft EULA or as verified acceptance provenance. No further server run should occur until the captain/user confirms an existing applicable acceptance record. The technical files remain real, but strict final validation must repeat the baseline after that confirmation; otherwise runtime validation is reported as blocked.
## Repeated baseline
...
That corrected validation was not run: the isolated templates contained `eula=true` without confirmed Leaf-Performance-specific acceptance provenance, so no further server start was permitted. The performance patch was removed rather than retaining an unvalidated experiment. No post-change performance result is claimed, and round 1 ends with **zero retained performance patches**. This is a validation-blocked rejection, not evidence that the discarded code was faster, slower or equivalent. Reconsidering it requires confirmed EULA provenance and the predeclared normal-20-TPS CPU experiment above.
